...
export function Footer() {
  ...
  return (
    ...
    >
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-4 px-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-6">
          {footerSlogans.map((item) => {
            ...
            return (
              ...
              >
                <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-footer-accent text-footer-bg shadow">
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-12">
          ...
          <div className="space-y-4 lg:col-span-4">
            ...
            <p className="text-xs leading-relaxed font-medium text-footer-muted">
            ...
            <div className="space-y-2 text-xs text-footer-muted">
              {hotline ? (
                <p className="flex items-center gap-2">
                  ...
                  <span className="text-footer-foreground">Hotline Đặt Bàn:</span>
                  <a
                    href={hotlineHref(hotline)}
                    className="font-bold text-footer-accent hover:underline"
                  >
                    {formatHotline(hotline)}
                  </a>
                </p>
              ...
              {email ? (
                <p className="flex items-center gap-2">
                  ...
                  <span className="text-footer-foreground">Email:</span>
                  <a href={`mailto:${email}`} className="font-semibold text-footer-accent hover:underline">
                    {email}
                  </a>
                </p>
              ) : null}
          ...
          <div className="space-y-3 lg:col-span-4">
            <h3 className="border-l-4 border-brand-red pl-3 text-xs font-black uppercase tracking-widest text-footer-accent">
              Hệ thống chi nhánh
            </h3>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive
                        ? "border-footer-accent bg-footer-foreground/10 shadow-lg ring-1 ring-footer-accent/50"
                        : "border-footer-foreground/20 bg-footer-foreground/5 hover:border-footer-accent/50",
                    )}
                  >
                    <div className="mb-1 flex items-center justify-between gap-2">
                      <span className="flex items-center gap-1.5 font-black text-footer-accent">
                    ...
                    <p className="font-medium text-footer-muted">
          ...
          <div className="space-y-3 lg:col-span-4">
            ...
            <div className="rounded-2xl border border-footer-foreground/20 bg-footer-foreground/5 p-2 shadow-xl">
              <div className="relative h-64 overflow-hidden rounded-xl bg-footer-foreground/10 sm:h-72">
                {mapEmbed ? (
                  ...
                  <div className="flex size-full flex-col items-center justify-center gap-3 px-6 text-center">
                    <MapPin className="size-12 text-footer-accent/70" />
                    <p className="text-sm font-bold text-footer-foreground">
                      {activeBranch?.name ?? "Chi nhánh"}
                    </p>
                    <p className="text-xs text-footer-muted">
                      Bản đồ sẽ hiển thị tại đây
                    </p>
                    <p className="text-[10px] text-footer-muted/80">
                      Cấu hình{" "}
                      <code className="text-footer-accent">
                        NEXT_PUBLIC_MAP_EMBED_{activeBranch?.mapKey.toUpperCase()}
                      </code>
                    </p>
                  </div>
                )}
                <div className="absolute right-2 bottom-2 left-2 rounded-lg border border-footer-accent/40 bg-footer/95 p-2 text-center text-[10px] font-bold text-footer-foreground backdrop-blur-sm">
      ...
      {/* Copyright */}
      <div className="border-t border-brand-red/50 bg-gradient-to-r from-slogan-mid via-brand-red to-footer py-4 text-center text-xs font-medium text-footer-muted">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 px-4 sm:flex-row">
          <p>
            ...
            {" | "}
            <span className="font-semibold text-footer-foreground">Tuyển Dụng Nhân Sự</span>
          </p>
        </div>
      </div>
    </footer>
